...
**Фікс: бот не відповідає на зображення без тексту**
- Проблема: коли Ксюша відправляла зображення без caption → в history записувався порожній content "" → при наступному запиті Anthropic API повертав 400 Bad Request (user messages must have non-empty content)
- Фікс 1 (core/ai.py): якщо в content є тільки зображення без текстового блоку → автоматично додається "Проаналізуй це зображення."
- Фікс 2 (bot/client.py): запис в history тепер перевіряє image_paths і ніколи не записує порожній content — мінімум "[зображення]"
